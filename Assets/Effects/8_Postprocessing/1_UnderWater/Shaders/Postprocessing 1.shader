Shader "Hidden/effectshader"
{
	Properties
	{
		...
		_UnderWaterColorProportion ("UnderWaterColorProportion", Range(0, 1)) = 0.3
	...
	SubShader
	{
	 ...
	 Cull Off ZWrite Off ZTest Always
		...
		Pass
		{
			...
			float _UnderWaterColorProportion;
			float _FogIntensity;
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				if(_ScreenParams.y - i.vertex.y < _Posy + heightOffset)
				{	
					...
					float4 depthFog = lerp(col, _UnderWaterColor, fog) * _DepthFogContrast;
					...
					return col;
					col.rgb = col.rgb * (1 - _UnderWaterColorProportion) + float3(_UnderWaterColor.rgb) * _UnderWaterColorProportion;
				}
				else 
				{
					float4 col = tex2D(_MainTex, i.uv);
