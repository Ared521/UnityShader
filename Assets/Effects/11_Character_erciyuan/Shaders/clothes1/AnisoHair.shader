Shader "Unlit/Hair"
{
    Properties
    {
        ...
        _AnisoOffsetMap ("Aniso Offset Map", 2D) = "white" {}
        _AnisoOffset ("Aniso Offset", Range(-1, 1)) = 0.5
        _AnisoOffsetStrength ("Aniso Offset Strength", Range(-1, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _AnisoOffsetMap;
            float4 _AnisoOffsetMap_ST;
            float _AnisoOffset;
            float _AnisoOffsetStrength;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                final_Diffuse = base_Color * _HairColor * diffuseLayer1_Color;
                // // 第二层上色
                ...
                // 高光 (Kajiya-Kay 各项异性)
                half2 uv_Offset = i.uv * _AnisoOffsetMap_ST.xy + _AnisoOffsetMap_ST.zw;
                half4 offsetNoise = tex2D(_AnisoOffsetMap, uv_Offset).r;
                offsetNoise = (offsetNoise * 2 - 1) * _AnisoOffsetStrength;
                half3 binormal_World_Offset = normal_World * (_AnisoOffset + offsetNoise);
                binormal_World = normalize(binormal_World + binormal_World_Offset);

                half3 H = normalize(light_World + view_World);
                half NdotH = saturate(dot(normal_World, H));
                
                // 不同引擎的切线方向跟UV是不同的，unity中u对应的是切线防线，v对应的是副切线方向，这里kajiya计算公式中的T，在unity对应的是binormal
                half HdotT = dot(H, binormal_World);
                half sinTH = sqrt(1 - HdotT * HdotT);
                half specular_Term = max(0.0001, pow(sinTH, _Specular_Smoothness));
                half3 final_Specular = specular_Term * _Specular_Intensity * _Specular_Color.rgb;


                half3 final_Color = final_Diffuse + final_Specular;
